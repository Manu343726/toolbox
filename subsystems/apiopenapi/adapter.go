...
func (a *Invoker) InvokeApi(ctx context.Context, request *connect.Request[apiv1.InvokeApiRequest]) (*connect.Response[apiv1.InvokeApiResponse], error) {
	...
	target, headers, body, err := a.buildRequest(server, described, operation, request.Msg.GetArgumentsJson())
	if err != nil {
		return nil, connect.NewError(connect.CodeInvalidArgument, err)
	}
	// The method is normalized here rather than trusted from the description: a
	// registered descriptor is data, and an HTTP method is case-insensitive on
	// the wire but conventionally uppercase.
	httpRequest, err := http.NewRequestWithContext(ctx, strings.ToUpper(strings.TrimSpace(operation.Method)), target, body)
	if err != nil {
		return nil, connect.NewError(connect.CodeInvalidArgument, fmt.Errorf("build request: %w", err))
	}
	...
	if err != nil {
		return nil, connect.NewError(connect.CodeUnavailable, fmt.Errorf("read response: %w", err))
	}
	truncated := len(payload) > maxResponseBytes
	if truncated {
		payload = payload[:maxResponseBytes]
	}
	encoded := normalizeJSON(payload, truncated)
	result := &apiv1.InvokeApiResponse{
		Status:      int32(response.StatusCode),
		ContentType: response.Header.Get("Content-Type"),
		Headers:     significantHeaders(response.Header),
		BodyJson:    encoded,
	}
...
func (a *Invoker) buildRequest(
	...
	for _, parameter := range operation.Parameters {
		...
		if !supplied {
			if parameter.Required {
				return "", nil, nil, fmt.Errorf("operation %q requires the %q parameter", operation.Name, parameter.Name)
			}
			continue
		}
	...
	if raw, ok := values["body"]; ok {
		consumed["body"] = true
		if !json.Valid(raw) {
			return "", nil, nil, fmt.Errorf("body must be valid JSON")
		}
	...
	for name := range values {
		...
		return "", nil, nil, fmt.Errorf("operation %q does not accept the argument %q", operation.Name, name)
	}
...
func applyParameter(path *string, parameter api.Parameter, raw json.RawMessage, query url.Values, headers http.Header) error {
	...
	case api.ParameterInPath:
		if text == "" {
			return fmt.Errorf("path parameter %q cannot be empty", parameter.Name)
		}
		placeholder := "{" + parameter.Name + "}"
		if !strings.Contains(*path, placeholder) {
			return fmt.Errorf("operation path %q does not contain the declared parameter %q", *path, parameter.Name)
		}
	...
	case api.ParameterInBody:
		return fmt.Errorf("body parameter %q must be supplied as the body argument", parameter.Name)
	default:
...
func parameterText(parameter api.Parameter, raw json.RawMessage) (string, error) {
	...
	if err := json.Unmarshal(raw, &text); err != nil {
		return "", fmt.Errorf("parameter %q must be a scalar value: %w", parameter.Name, err)
	}
	return text, nil
}
	...
	for _, requirement := range requirements {
		...
		if !found {
			if len(target.SecuritySchemes) == 0 {
				...
				// satisfied knowingly.
				return fmt.Errorf("operation %q requires security scheme %q, which the document does not describe", operation.Name, requirement.Scheme)
			}
			continue
		}
		if a.credentials == nil {
			return fmt.Errorf("operation %q requires security scheme %q, and this deployment supplies no credentials", operation.Name, requirement.Scheme)
		}
		credential, ok := a.credentials.Credential(scheme, requirement)
		if !ok || credential == "" {
			return fmt.Errorf("operation %q requires security scheme %q, and no credential is available for it", operation.Name, requirement.Scheme)
		}
		switch strings.ToLower(scheme.Type) {
		case "apikey", "api_key":
			name := scheme.ParameterName
			if name == "" {
				return fmt.Errorf("api key scheme %q does not name the parameter it travels under", scheme.Name)
			}
		...
		default:
			return fmt.Errorf("operation %q requires security scheme %q of unsupported type %q", operation.Name, requirement.Scheme, scheme.Type)
		}
	}
	return nil
}
